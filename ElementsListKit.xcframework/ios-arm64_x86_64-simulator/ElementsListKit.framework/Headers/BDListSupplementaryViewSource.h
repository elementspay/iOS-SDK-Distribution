...
/**
 ...
 `BDListAdapter` which then configures and maintains a `UICollectionView`. The supplementary API reflects that in
 `UICollectionView`, `UICollectionViewLayout`, and `UICollectionViewDataSource`.
 */
NS_SWIFT_NAME(ListSupplementaryViewSource)
@protocol BDListSupplementaryViewSource <NSObject>
...
/**
 ...
 @warning You should never allocate new views in this method. Instead deque a view from the `BDListCollectionContext`.
 */
