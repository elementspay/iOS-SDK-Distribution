...
/**
 ...
 It then automatically binds each view model to cells via the `BDListBindable` protocol.

 Models used with `BDListBindingSectionController` should take special care to always return `YES` for identical
 ...
 In Swift:
 ```
 func isEqual(toDiffableObject object: BDListDiffable?) -> Bool {
   return true
 }
 ```
...
@interface BDListBindingSectionController<__covariant ObjectType : id<BDListDiffable>> : BDListSectionController

/**
 A data source that transforms a top-level object into view models, and returns cells and sizes for given view models.
 */
@property (nonatomic, weak, nullable) id<BDListBindingSectionControllerDataSource> dataSource;

/**
 A delegate that receives selection events from cells in an `BDListBindingSectionController` instance.
 */
@property (nonatomic, weak, nullable) id<BDListBindingSectionControllerSelectionDelegate> selectionDelegate;
 ...
 */
@property (nonatomic, copy, readonly) NSArray<id<BDListDiffable>> *viewModels;
