...
#import <ElementsListKit/BDListAdapterMoveDelegate.h>
#import <ElementsListKit/BDListAdapterPerformanceDelegate.h>
#import <ElementsListKit/BDListAdapterUpdateListener.h>
#import <ElementsListKit/BDListCollectionContext.h>

@protocol BDListUpdatingDelegate;
...
typedef void (^BDListUpdaterCompletion)(BOOL finished);

/**
 `BDListAdapter` objects provide an abstraction for feeds of objects in a `UICollectionView` by breaking each object
 into individual sections, called "section controllers". These controllers (objects subclassing to
 `BDListSectionController`) act as a data source and delegate for each section.

 Feed implementations must act as the data source for an `BDListAdapter` in order to drive the objects and section
 controllers in a collection view.
 */
IGLK_SUBCLASSING_RESTRICTED
NS_SWIFT_NAME(ListAdapter)
@interface BDListAdapter : NSObject
 ...
 */
@property (nonatomic, nullable, weak) id <BDListAdapterDataSource> dataSource;

/**
 The object that receives top-level events for section controllers.
 */
@property (nonatomic, nullable, weak) id <BDListAdapterDelegate> delegate;
...
/**
 The object that receives `BDListAdapterMoveDelegate` events resulting from interactive reordering of sections.

 @note This works with UICollectionView interactive reordering available on iOS 9.0+
 */
@property (nonatomic, nullable, weak) id <BDListAdapterMoveDelegate> moveDelegate NS_AVAILABLE_IOS(9_0);

/**
 The object that receives `BDListAdapterPerformanceDelegate` events to measure performance.
 */
@property (nonatomic, nullable, weak) id <BDListAdapterPerformanceDelegate> performanceDelegate;

/**
 The updater for the adapter.
 */
@property (nonatomic, strong, readonly) id <BDListUpdatingDelegate> updater;

/**
 A bitmask of experiments to conduct on the adapter.
 */
@property (nonatomic, assign) BDListExperiment experiments;

/**
 Initializes a new `BDListAdapter` object.
 ...
 */
- (instancetype)initWithUpdater:(id <BDListUpdatingDelegate>)updater
                 viewController:(nullable UIViewController *)viewController
               workingRangeSize:(NSInteger)workingRangeSize NS_DESIGNATED_INITIALIZER;

/**
 Initializes a new `BDListAdapter` object with a working range of `0`.
 ...
 */
- (void)performUpdatesAnimated:(BOOL)animated completion:(nullable BDListUpdaterCompletion)completion;
...
/**
 ...
 section controllers. Use `-[BDListAdapter performUpdatesAnimated:completion]` instead.
 */
- (void)reloadDataWithCompletion:(nullable BDListUpdaterCompletion)completion;
 ...
 */
- (nullable BDListSectionController *)sectionControllerForSection:(NSInteger)section;
 ...
 */
- (NSInteger)sectionForSectionController:(BDListSectionController *)sectionController;
...
/**
 ...
 @see `-[BDListAdapterDataSource listAdapter:sectionControllerForObject:]`
 */
- (__kindof BDListSectionController * _Nullable)sectionControllerForObject:(id)object;
 ...
 */
- (nullable id)objectForSectionController:(BDListSectionController *)sectionController;
 ...
 */
- (NSArray<BDListSectionController *> *)visibleSectionControllers;
...
/**
 ...
 @param updateListener The object conforming to the `BDListAdapterUpdateListener` protocol.

 @note Listeners are held weakly so there is no need to call `-[BDListAdapter removeUpdateListener:]` on `dealloc`.
 */
- (void)addUpdateListener:(id<BDListAdapterUpdateListener>)updateListener;
...
/**
 ...
 @param updateListener The object conforming to the `BDListAdapterUpdateListener` protocol.
 */
- (void)removeUpdateListener:(id<BDListAdapterUpdateListener>)updateListener;
